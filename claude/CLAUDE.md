...
- **When Adam edits an em dash out of a draft, match the replacement to the pattern, not just "remove the dash."** Semicolon when it was joining two closely-related independent clauses (*"Avro is contract per #463 — replacing Avro would break every consumer"* → *"Avro is contract per #463; replacing Avro would break every consumer"*). Comma when it was setting off a looser parenthetical or a non-independent trailing clause (*"the specific registry vendor — the page just didn't say so"* → *"the specific registry vendor, the page just didn't say so"*). A sentence break when the em-dashed clause could stand alone. A colon when it was introducing a list or a specific example.

- **Prefer active voice for anything Adam sends.** Passive voice hides the agent; naming it (Adam, the ADR, the team) reads as sharper and more owned. *"The pattern's applied consistently"* → *"I've applied the pattern consistently"*; *"is deferred to"* → *"defers to"*. Applies to the same peer-facing drafts as the em-dash rule. Doesn't apply where the passive is load-bearing at the org level and no single agent is the actor (e.g., an ADR: *"authorization decisions are made against a uniform model"*).
